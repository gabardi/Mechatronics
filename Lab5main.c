...
Project : Timer/Interrupts and Data Structures
Version : 01
Date    : 10/1/2019
...
Description:
The goal of this lab is to get hands-on experience
implementing timers and interrupts. The program created
will configure a 16-bit and 8-bit timer counter to
interrupt at different rates.

The end of this code practices how to create data
structures, and how access larger data types,
in addition to exploring unions to create single
data objects.
*******************************************************/
...
// Place your code here
	PORTB = ~PORTB;		//Chose output pin to measure frequency
						//Inverts output every interrupt
}
...
{
	// Place your code here
	PORTA = ~PORTA;		//Chose output pin to measure frequency
						//Inverts output every interrupt
}

/////*** Question 8 ***/////
// Creates a data structure that stores two char values
struct twobytes
{
	unsigned char lsb;
	unsigned char msb;
...
// Creates a union to create a single data object consisting of the
// variables lsb and msb
union byteint
{
...
/////*** Question 10 ***/////
// Creates a structure with two members structure twobytes
// Allows byte access to a long integer value
struct fourbytes
{
	struct twobytes char1;		//least significant byte pair
	struct twobytes char2;		//most significant byte pair
	
} bytes2;

//Overlays long int on structure fourbytes' members
union bytelong
{
	struct fourbytes bytes2;
	unsigned long intvallong;
};

/////*** Question 12 ***/////
// Creates a union to overlay an array of unsigned 8-bit
// values with a floating point variable
union bytefloat
{
	char floatholder[8];
	float intvalfloat;
...
void main(void)
{
	...
	//union bytelong distance;
	//Declare test variables/structures for Q13
	union bytefloat displacement;
	float variable;
	
	// Validation code to determine correct endian for question 9.
	// Setting the byes in the union byteint to verify output should be 0xBF80
	//position.intval = 0xBF80;
	//position.bytes.lsb = 0x80;
	//position.bytes.msb = 0xBF;

	// Q11
	// Validation code to determine correct endian for question 10.
	// Setting the byes in the union bytelong to verify output should be 0x44332211

	//distance.bytes2.char1.lsb = 0x11;
	//distance.bytes2.char1.msb = 0x22;
	//distance.bytes2.char2.lsb = 0x33;
	//distance.bytes2.char2.msb = 0x44;


	//Q13

	// Set a variable equal to floating point value and determine type of precision floating point
	// observe the outputted byte pattern as an array
	variable = 3.14159;
	displacement.intvalfloat = variable;
...
#asm("sei")		//Enable Global Interrupts (Assembly Language)
