...
Description: 
The goal of this lab is to practice reading an A/D and
converter, writing to an A/D converter, and exploring 
the problems of aliasing. In the various exercises,
the reference input signal and the new signal are 
analyzed with a DSA in both the time domain and in the 
frequency domain.
*******************************************************/
...
#define ANALOGIN14	0b00100110	//Analog In 14 Channel
...
/**********************************************************
Function that takes in channel number and an 8-bit value
and writes it to the D/A converter. The function will then
output the 8-bit value to the corresponding D/A channel. If
an invalid channel number is used, the function with return
a one indicating an error. Valid channels are 0-3.
**********************************************************/
char Write2DAC(unsigned char channel, unsigned char data){
	//Execute sequence only if channel number is valid
	if((channel >= 0) && (channel <= 3)){
		ADDRESSBUS = 0x20 + channel;	//Address to Disable DAC i channel
		DATASTATUS = 0xFF;				//Change PORTD to output
		
		DATABUSOUT = data;				//Send data on data bus
		delay_us(1);					//Wait for data bus to stabilize
		
		ADDRESSBUS = channel;			//Adress to enable DAC i channel
		delay_us(1);					//Wait for data to propagate
		
		ADDRESSBUS = 0x20 + channel;	//Disable DAC channel
		
		return 0x00;					//If sequence executes, return no error
	}
	
	else{
		return 0x01;					//If invalid channel number return error
	}
}

// Declare your global variables here
unsigned char counter0,counter1,counter2,counter3;		//counter for each DAC channel
char error;												//error variable for invalid channel 
unsigned int DigitalOut;								//store 10-bit value out of ADC
...
/********************************************************
Test Code for Question 14 - Output a sawtooth waveform
The following code writes to each of the four channels 
sequentially with varying values of increments to 
observe a different saw tooth waveform at every channel
********************************************************/
	/*
	error = Write2DAC(0,counter0);
    counter0 = counter0 +1;
	
	error = Write2DAC(1,counter1);
    counter1 = counter1 +2;
	
	error = Write2DAC(2,counter2);
    counter2 = counter2 +3;
	
	error = Write2DAC(3,counter3);
    counter3 = counter3 +4;
	*/

	//Read and store output of ADC14
	DigitalOut = read_adc(ANALOGIN14);
	
	//Right sift two bits to store 8 most significant bits
	//from the 10-bit output of the ADC and write it to the
	//DAC on the daughter board.
	error = Write2DAC(1,(DigitalOut>>2));
